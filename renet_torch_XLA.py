...
test_loader = DataLoader(test_dataset, batch_size=1024, sampler=test_sampler,
                         follow_batch=['h_sub', 'h_obj'], num_workers=6)
...
def _mp_fn(rank, flags):
    ...
    for epoch in range(1, 21):
        ...
        # XLA-Specific print
        xm.master_print(f'Epoch: {epoch:02d}, MRR: {mrr:.4f}, Hits@1: {hits1:.4f}, '
                        f'Hits@3: {hits3:.4f}, Hits@10: {hits10:.4f}')
        
        times.append(time.time() - start)
    xm.master_print(f"Median time per epoch: {torch.tensor(times).median():.4f}s")
